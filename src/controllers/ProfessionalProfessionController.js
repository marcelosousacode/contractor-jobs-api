...
module.exports = {
    async index(req, res) {
        ...
        INNER JOIN profession ON profession.id = item_professional_profession.fk_profession;`, (err, rows) => {
            if(err) throw err
    ...
    async selectProfessionalsByProfession(req, res) {
        ...
        INNER JOIN professional ON professional.id = item_professional_profession.fk_professional
        INNER JOIN profession ON profession.id = item_professional_profession.fk_profession WHERE profession.id = ?;`, [
            req.params.id
        ], (err, rows) => {
            if (err) throw err
            return res.json(rows)
